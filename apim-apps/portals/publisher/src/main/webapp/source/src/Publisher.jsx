...
import JsonLocate from '../../site/public/locales/vi.json'
 ...
 */
const languageWithoutRegionCode = 'en'// language.toLowerCase().split(/[_-]+/)[0];
...
class Publisher extends React.Component {
    ...
    loadLocale(locale) {
        ...
        if (locale !== 'en' || Configurations.app.loadDefaultLocales) {
            const messages = JsonLocate
            this.setState({ messages })
            // fetch(`${Configurations.app.context}/site/public/locales/${locale}.json`)
            // // fetch(`${Configurations.app.context}/site/public/locales/en.json`)
            //     .then((resp) => resp.json())
